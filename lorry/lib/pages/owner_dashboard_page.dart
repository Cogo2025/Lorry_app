...
class OwnerDashboardPage extends StatefulWidget {
  ...
  const OwnerDashboardPage({Key? key, required this.ownerName})
      : super(key: key);
...
class _OwnerDashboardPageState extends State<OwnerDashboardPage> {
  ...
  void _onNavigationItemSelected(int index) {
    ...
    if (index == 1) {
      // Navigate to OwnerProfile when person icon is selected
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => OwnerProfile()),
      );
    } else if (index == 3) {
      // Navigate to CustomLayoutPage when favorite icon is selected
      Navigator.push(
        context,
